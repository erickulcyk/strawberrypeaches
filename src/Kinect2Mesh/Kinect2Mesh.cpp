...
void SavePCD(string name, pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud, bool binary)
{
	cout<<"Saving to pcd: "<<name<<" "<<(cloud==NULL)<<" "<<binary<<endl;
	pcl::io::savePCDFile (name,*cloud, binary);
}

void ReadCloud (pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud, ifstream& file, int index, bool useBGR, bool saveToPcd)
{
	char dsize[4];
	file.read (dsize, 4);
	...
	if(saveToPcd)
	{
		...
		sstream << "cloud_"<<index<<".pcd";
		SavePCD(sstream.str(),cloud,true);
	}
...
{
	cout<<"Usage: [# iterations], [Use rgb], [ply file name], [use ICP] [save ply]"<<endl;
	...
	pcl::PointCloud<pcl::PointXYZRGBA>::Ptr clouddummy (new pcl::PointCloud<pcl::PointXYZRGBA>); 
	...
	ifstream file ("../data/KinectDepth3.txt", ios::in|ios::binary|ios::ate);
	...
	if (file.is_open())
	{
		...
		for(int  i=0; i <iters; i++)
		{
			if(i==0)
			{
				ReadCloud(cloud, file,i, useBGR,true);
				//FilterCloud(cloud);
				while(true)
				{
			...
			{
				ReadCloud(cloud2, file,i, useBGR, true);
				//FilterCloud(cloud2);
				ShowCloud(cloud2,i);
				if(useICP)
				{
